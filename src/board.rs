...
pub const LED1: u8 = 0x10; // PK4
const LED2: u8 = 0x40; // PK6

const LED3: u8 = 0x01; //PN0

//see ADC signals, p.1053
const FD_ADC:   u8 = 0x01; // PE0, AIN3
const FV_ADC:   u8 = 0x02; // PE1, AIN2
const FBI_ADC:  u8 = 0x04; // PE2, AIN1
const IC_ADC:   u8 = 0x08; // PE3, AIN0
const FBV_ADC:  u8 = 0x20; // PD5, AIN6
const AV_ADC:   u8 = 0x40; // PD6, AIN5
...
pub fn init() {
    cortex_m::interrupt::free(|_cs| {
        ...
        */
        ...
        while !sysctl.prgpio.read().r3().bit() {} //port D
        while !sysctl.prgpio.read().r4().bit() {} //port E
        ...
        let gpio_n = unsafe { &*tm4c129x::GPIO_PORTN::ptr() };
        gpio_n.dir.write(|w| w.dir().bits(LED3|0x02));
        gpio_n.den.write(|w| w.den().bits(LED3|0x02));
        /*
    ...
    */
       ...
       /*
        if adc0.ostat.read().ov0().bit() {            
            adc0.ostat.write(|w| w.ov0().bit(true));
        }
        */
